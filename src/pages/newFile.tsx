...
;<Fragment>
	<Layout title="Tip Calculator App">
		<div class="bg-skin-canvas-pri pt-12 text-bodyS">
			<Header classes="flex justify-center pb-8" />
			<main class="bg-skin-brand-tert pt-8 max-w-md rounded-lg py-6 px-4 mx-auto">
				<div>
					<Input
						for="bill"
						labelName="Bill"
						...
						pattern="^\\d+(\\.)?(\\.\\d{1,2})?$"
						placeholder="0"
						errorMsg="Must be a number, no more than two decimals"
						dataName="bill">
						<svg
				...
				<div class="mt-8 py-8 px-6 rounded-xl bg-skin-brand-pri">
					<TipDisplays></TipDisplays>
				</div>
			</main>
		</div>
	</Layout>
		...
		{() => {
			...
			btnGroup.forEach((element) => {
				element.addEventListener("click", (e) => {
					...
					tipData.tipPercent = e.target.value
					console.log(calcTip(tipData.bill, tipData.tipPercent))
				})
			})
